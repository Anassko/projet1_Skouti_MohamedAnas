...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $updateAddressQuery = "UPDATE address
                           SET street_name = ?, street_nb = ?,
                               ...
                               zip_code = ?, country = ?
                           WHERE id = ?";
    $stmt = $con->prepare($updateAddressQuery);
    $stmt->bind_param("sissssi", $newStreetName, $newStreetNumber, $newCity, $newProvince, $newZipCode, $newCountry, $userData['billing_address_id']);
    $stmt->execute();
    $stmt->close();
    ...
    <div class="inner">
        <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post">
            ...
            <div class="form-wrapper">
                ...
                <input type="number" name="zipcode" value="<?php echo $userData['zip_code']; ?>" class="form-control" required>
            </div>
